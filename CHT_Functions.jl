...
function CreateMesh(case_dir,input_stp,plotter)
    ...
    gmsh.write(case_dir* "/" *"coldplate.msh")
...
function SolveCHT(case_dir)

    # Flow Solve
    flowrate = (1.65e-3/60)*1e9
    μ = 1e-3

    model = GmshDiscreteModel(case_dir* "/" *"coldplate.msh")
    ...
    GridapPETSc.with(args=options) do
        ...
        writevtk(Ω_fluid,case_dir* "/" *"stokes_results", cellfields=["u"=>uh, "p"=>ph])
        ...
        save(case_dir* "/" *"coldplate_stokes.png", fig)
        println("Stokes plot saved")
   

        #FLUID TEMPERATURE WITH VOLUMETRIC HEAT SOURCE
        k_f = 0.6e-3
        ρ = 1.0e-6
        cp = 4186.0
        T_inlet = 40.0 + 273.15
        ...
        a_f(tf, sf) =
            ∫( k_f*(∇(tf)⋅∇(sf)) + ρ*cp*((uh⋅∇(tf))*sf) + τ*(uh⋅∇(sf))*(uh⋅∇(tf)) )dΩ_f

        l_f(sf) =
            ∫( Q_vol * sf + τ*(uh⋅∇(sf))*Q_vol )dΩ_fluid_fin
        ...
        heat_out = sum(∫( ρ*cp*(uh⋅get_normal_vector(Γ_outlet_geom))*Th_f )dΓo_t)
        heat_in  = sum(∫( ρ*cp*(uh⋅get_normal_vector(Γ_inlet_geom))*Th_f )dΓi_t)
        println("Net convective heat transport: ", heat_out + heat_in, " W")
        println("Expected: 1100 W")

        writevtk(Ω_fluid, case_dir* "/" *"temperature_fluid", cellfields=["T"=>Th_f])

        fig = Figure(size=(800, 500))
        ax = Axis3(fig[1,1], title="Fluid temperature")
        p = plot!(ax, Ω_fluid, Th_f)
        Colorbar(fig[2,1], p, vertical=false, label="T (K)")
        save(case_dir* "/" *"coldplate_temperature.png", fig)
